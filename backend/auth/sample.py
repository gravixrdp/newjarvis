...
# Ensure samples directory exists (in case a file with same name existed earlier)
samples_dir = os.path.join('backend', 'auth', 'samples')
if not os.path.isdir(samples_dir):
    # If a file exists with this name, remove it, then create the directory
...
while True:
    ...
    for (x,y,w,h) in faces:
        ...
        cv2.imwrite(os.path.join(samples_dir, f"face.{face_id}.{count}.jpg"), converted_image[y:y+h,x:x+w])
